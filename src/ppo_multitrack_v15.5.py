#!/usr/bin/env python3
"""
DonkeyCar PPO V15.5 (multi-track)
...
- V18 hyperparams: lstm_hidden=256, n_steps=4096, batch=256, auto_lr_min=5e-5
"""
...
SCENE_SPECS: Dict[str, Dict[str, Any]] = {
    ...
    # --- RRL note(V15.5 note ws+gt notetraining) ---
...
def _resolve_track_dir(track_dir: Optional[str], env_ids: List[str]) -> str:
    ...
    for candidate in raw_candidates:
        ...
        candidate_summaries.append(
            f"{candidate_abs} (missing: {', '.join(missing)})"
        )
...
# ============================================================
# Curriculum stages - V15.5 note ws+gt, RRL note
# ============================================================
STAGE_ENV_IDS: Dict[str, List[str]] = {
    "S1": [
        "donkey-waveshare-v0",
        "donkey-generated-track-v0",
        # "donkey-roboracingleague-track-v0",   # rrl note
    ],
    "S2": [
        "donkey-waveshare-v0",
        "donkey-generated-track-v0",
...
def train_v13(
    ...
    save_dir: str = "models/v15_5_multi_scene",
    port: int = 9091,
    # obs
    obs_size: int = 128,
    augment: bool = False,
    ...
    for eid in env_ids:
        if eid not in SCENE_SPECS:
            raise KeyError(f"Unsupported env_id for V15.5: {eid}")
    ...
    if not use_lstm:
        raise ValueError("V15.5 requires RecurrentPPO MultiInputLstmPolicy; set --use-lstm")
    ...
    print("\n" + "=" * 76)
    print("🚀 DonkeyCar PPO V15.5 - 2-Domain Multi-Scene Recurrent Training")
    print("=" * 76)
    ...
    try:
        ...
        if cfg is not None and hasattr(cfg, "GYM_CONF"):
            ...
            _update = {
                "port": port,
                "car_name": "waveshare_v15_5",
                "racer_name": "V15.5-ActionAdapter",
                "country": "CN",
                "bio": "V15.5 3D action adapter with FiLM",
                "guid": "waveshare-v15-5-multi",
                "max_cte": 8.0,
            }
            if _launch_sim:
    ...
    if resume_path:
        ...
        resume_ckpt_path = _find_latest_checkpoint(save_dir, name_prefix="v15_5")
        if resume_ckpt_path is None:
            raise FileNotFoundError(f"No v15_5 checkpoint found in {save_dir}")
    ...
    print("🏗️  Building multi-scene V15.5 env")
    ...
    callbacks: List[BaseCallback] = [
        PTHExportCallback(save_path=save_dir, save_freq=20000, name_prefix="v15_5", verbose=1),
        BestModelCallback(
            save_path=save_dir,
            check_freq=1000,
    ...
    config = {
        ...
        "observation": {
            ...
            "image_channels": [
                "raw_Y", "edge_line_prob", "guide_line_prob",
    ...
    config_path = os.path.join(save_dir, "v15_5_config.json")
    with open(config_path, "w", encoding="utf-8") as f:
        json.dump(config, f, indent=2, ensure_ascii=False)

    print("\n" + "=" * 76)
    print("PASS V15.5 training finished")
    print("=" * 76)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="V15.5: 2-domain recurrent PPO with 6ch semantic obs, 7D state, 3D action adapter",
    ...
    parser.add_argument("--stage", type=str, default=None, choices=["S1", "S2"],
                        help="notestagetraining: S1(ws+gt+rrlnote) / S2(ws+gt+rrl).note --env-ids")
    ...
    parser.add_argument("--save-dir", type=str, default="models/v15_5_multi_scene")
    ...
    parser.add_argument("--auto-lr-min", type=float, default=5e-5)
    ...
    parser.add_argument("--auto-lr-balanced-drop", type=float, default=8.0)
    ...
    print(
        f"🆕 V15.5 profile={args.train_profile} | "
    ...
    if use_curriculum_auto:
        ...
        if effective_env_ids is None and args.stage is None:
            print("ℹ️  note --env-ids/--stage: note DEFAULT_ENV_IDS (ws+gt+rrl 3 note).")
